...
class CuriosityRepository(private val db: AppDatabase) {
    ...
    suspend fun initializeSeedData() {
        ...
        val curiosities = listOf(
            ...
            Curiosity(title = "Gli elefanti non sanno saltare",
                ...
                category = "Animali", emoji = "🐘"),
            Curiosity(title = "Oxford è più antica dell'Impero Azteco",
        ...
        val questions = listOf(
            QuizQuestion(curiosityId = ids[0].toInt(),
                ...
                wrongAnswer1  = "Circa 2 anni", wrongAnswer2 = "10-15 anni al massimo",
            ...
            QuizQuestion(curiosityId = ids[1].toInt(),
                ...
                wrongAnswer1  = "1 cuore", wrongAnswer2 = "2 cuori", wrongAnswer3 = "4 cuori",
    ...
    suspend fun salvaNota(c: Curiosity, nota: String) =
    ...
    suspend fun getPerRipasso(giorniMin: Int, categorie: Set<String> = emptySet()): List<Curiosity> {
        val (lista, tutte) = categorie.toFiltro()
        return if (giorniMin == 0) {
            curDao.getTutteImparate(lista, tutte)
        } else {
            val soglia = System.currentTimeMillis() - TimeUnit.DAYS.toMillis(giorniMin.toLong())
            curDao.getPerRipasso(soglia, lista, tutte)
        }
    }

    // ── Quiz ──────────────────────────────────────────────────────────────────

    suspend fun getQuizQuestionsWithCategory(n: Int, categorie: Set<String> = emptySet()): List<QuizQuestionWithCategory> {
        val (lista, tutte) = categorie.toFiltro()
    ...
    suspend fun statPerCategoria()  = sessionDao.statPerCategoria()
    suspend fun ultime20Sessioni()  = sessionDao.ultime20()

    // ── Preferiti ─────────────────────────────────────────────────────────────

    suspend fun cercaBookmark(query: String = "", categorie: Set<String> = emptySet()): List<Curiosity> {
        val (lista, tutte) = categorie.toFiltro()
        return bookmarkDao.cerca(query, lista, tutte)
    }

    suspend fun categorieBookmark()           = bookmarkDao.categorieBookmark()
    suspend fun rimuoviBookmark(c: Curiosity) = bookmarkDao.update(c.copy(isBookmarked = false))

    // ── Badge ─────────────────────────────────────────────────────────────────

    suspend fun sbloccaBadge(badge: BadgeSbloccato) = badgeDao.sblocca(badge)
    suspend fun badgeSbloccati()                     = badgeDao.tutti()
    suspend fun idBadgeSbloccati()                   = badgeDao.idSbloccati()
    ...
    suspend fun quizNonRisposti(): Int {
        val (lista, tutte) = emptySet<String>().toFiltro()
